//! 32-state split: (prev==0?0:16)+(prev==1?0:8)+class8, SCALE=1024, skip empty ctx.

use crate::ans::{RansEnc, RANS_L};

pub const MAGIC: &[u8; 4] = b"CL32";
...
pub fn ctx(prev: u8) -> usize {
    let hi = if prev == 0 { 0 } else { 16 };
    let mid = if prev == 1 { 0 } else { 8 };
    let lo = match prev {
        0 | 1 => 0,
        2 | 3 => 1,
        4 | 5 => 2,
...
pub fn encode(mtf: &[u8]) -> Vec<u8> {
    ...
    for (c, (freq, _)) in tables.iter().enumerate() {
        ...
        let used: Vec<(u8, u16)> = (0..N_SYM)
            .filter(|&s| freq[s] > 0)
            .map(|s| (s as u8, freq[s].min(0x03FF) as u16))
            .collect();
