//! 8-state split: binary O1 on zero-bit + 8 order-0 tail tables.
//! ctx = (prev==0 ? 0 : 4) + class(prev). Value-buckets 64+ are empty after RLE0.

use crate::ans::{RansEnc, RANS_L, SCALE, SCALE_BITS};

pub const MAGIC: &[u8; 4] = b"CLN8";
const N_CTX: usize = 8;
const N_SYM: usize = 67;
const ESC: usize = 66;

pub fn ctx(prev: u8) -> usize {
    let hi = if prev == 0 { 0 } else { 4 };
    let lo = match prev {
        0 | 1 => 0,
        2 | 3 => 1,
        4..=15 => 2,
...
pub fn decode(buf: &[u8]) -> Result<Vec<u8>, &'static str> {
    ...
    for c in 0..N_CTX {
        if pos >= buf.len() {
            return Err("cln8: n_used");
        }
        let n_used = buf[pos] as usize;
        pos += 1;
        for _ in 0..n_used {
            if pos + 3 > buf.len() {
                return Err("cln8: freq");
            }
    ...
    pos += 4;
    if pos + tlen > buf.len() {
        return Err("cln8: tstream");
    }
    let tstream = &buf[pos..pos + tlen];
    if zlen < 8 || tlen < 8 {
        return Err("cln8: state");
    }
